// -----------------------------------------------------------------------------
// ComplaintFormScreen – full implementation (COMPLETE FILE)
// -----------------------------------------------------------------------------
...
import { useGenders } from '../api/lookup';
...
const FormInput = React.memo(
  React.forwardRef(
    (
      { label, isRequired, onFocus, multiline = false, numberOfLines = 1, ...props },
      ref,
    ) => {
        ...
        {steps.map((_, i) => (
          <React.Fragment key={i}>
            <View style={[styles.step, currentStep > i && styles.activeStep]}>
              <Text style={[styles.stepText, currentStep > i && styles.activeStepText]}>
                {i + 1}
              </Text>
            </View>
...
const Step3_ComplaintDetails = ({ onFieldChange, formData, onInputFocus, inputRefs }) => {
  ...
  const removeAttachment = i =>
    onFieldChange(
      'attachments',
      formData.attachments.filter((_, idx) => idx !== i),
    );
      ...
      <View style={styles.confirmationSection}>
        ...
        {formData.complainantType === 'individual' ? (
          ...
          <>
            ...
            <Text style={styles.confirmationItem}>رقم السجل التجاري: {formData.companyRC}</Text>
...
const ComplaintFormScreen = () => {
  ...
  const handleSubmit = () => {
    ...
    if (formData.complainantType === 'legal') {
      // Omit fields that would be null; let backend handle company mode
      plaignant = {
  ...
  const renderStep = () => {
    switch (currentStep) {
      case 1:
        return <Step1_ComplainantInfo {...stepProps} />;
      case 2:
        return <Step2_RespondentInfo {...stepProps} />;
      case 3:
        return <Step3_ComplaintDetails {...stepProps} />;
      case 4:
        return <Step4_Confirmation {...stepProps} />;
      default:
        return null;
    }
  };
      ...
      <KeyboardAvoidingView
        ...
        <View style={styles.footer}>
          ...
          <View style={styles.footerButtons}>
            <TouchableOpacity
              style={[styles.nextButton, isPending && styles.disabledButton]}
              onPress={next}
              disabled={isPending}>
            ...
            {currentStep > 1 && (
              <TouchableOpacity
                style={[styles.prevButton, isPending && styles.disabledButton]}
                onPress={prev}
                disabled={isPending}>
...
const styles = StyleSheet.create({
  ...
  attachmentItem: {
    ...
    justifyContent: 'space-between',
    alignItems: 'center',
    backgroundColor: 'rgba(58, 79, 83, 0.05)',
    borderRadius: 8,
    paddingVertical: 8,
    paddingHorizontal: 12,
    marginBottom: 8,
